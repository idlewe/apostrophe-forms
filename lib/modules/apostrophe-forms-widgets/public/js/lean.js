...
apos.utils.widgetPlayers['apostrophe-forms'] = function (el, data, options) {
  ...
  function submit(event) {
    ...
    if (recaptchaSlot) {
      ...
      var token = window.smartCaptcha.getResponse();
      if (!token) {
    ...
    return apos.utils.post('/modules/apostrophe-forms-ya/submit', event.input, function (err, res) {
      ...
      if (err || (res && (res.status !== 'ok'))) {
        ...
        apos.utils.emit(document.body, 'apostrophe-forms:submission-form', {
          form: form,
          res: res
        });
  ...
  function addRecaptchaScript () {
    ...
    var container = document.querySelector('[data-apos-refreshable]') || document.body;
    var recaptchaScript = document.createElement('script');
    recaptchaScript.src = 'https://smartcaptcha.yandexcloud.net/captcha.js?render=onload&onload=smartCaptchaInit';
    recaptchaScript.setAttribute('data-apos-recaptcha-script', '');
    recaptchaScript.setAttribute('async', '');
    recaptchaScript.setAttribute('defer', '');
    container.appendChild(recaptchaScript);

    setTimeout(()=>this.renderCaptchas(),500)
  }
  ...
  function renderCaptchas () {
    ...
    recaptchaSlots.forEach(function(slot) {
      var slotId = window.smartCaptcha.render(
